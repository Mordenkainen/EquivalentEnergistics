    ...
    public void updateDisplay() {
        ...
        if (hostGrid.getCurrentEMC() > 0) {
            ...
            for (int i = 4; i >= 0; i--) {
                ...
                if (crystalcount > 0) {
                    cachedList.add(AEApi.instance().storage().createItemStack(new ItemStack(ModItems.CRYSTAL, 1, i)).setStackSize(crystalcount));
                    remainingEMC -= crystalcount * crystalEMC;
                }
            }
        }

        cachedList.add(AEApi.instance().storage().createItemStack(new ItemStack(ModItems.MISC, 1, 1)).setStackSize((long) hostGrid.getCurrentEMC()));
        storageGrid.postAlterationOfStoredItems(StorageChannel.ITEMS, cachedList, new BaseActionSource());
    }
