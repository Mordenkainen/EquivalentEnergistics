...
public class ItemCellCreative extends ItemCellBase {
    ...
    public <T extends IAEStack<T>> ICellInventoryHandler<T> getCellInventory(final ItemStack stack, final ISaveProvider host, final IStorageChannel<T> channel) {
        if (channel == AEApi.instance().storage().getStorageChannel(IEMCStorageChannel.class) && isCell(stack)) {
            return (ICellInventoryHandler<T>) new HandlerEMCCellCreative(host);
        }
        return null;
    }

    @Override
    public <T extends IAEStack<T>> int getStatusForCell(final ItemStack stack, final ICellInventoryHandler<T> inv) {
        return 1;
    }
    
    @Override
    public <T extends IAEStack<T>> double cellIdleDrain(final ItemStack stack, final ICellInventoryHandler<T> inv) {
        return 0;
    }
