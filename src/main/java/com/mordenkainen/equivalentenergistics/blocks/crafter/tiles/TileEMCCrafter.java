...
public class TileEMCCrafter extends TileAEBase implements IGridTickable, IDropItems, ITransProvider, ICraftingProvider, ICraftingMonitor, IWailaNBTProvider {
    ...
    public boolean pushPattern(final ICraftingPatternDetails patternDetails, final InventoryCrafting invCrafting) {
        if (isActive() && patternDetails instanceof EMCCraftingPattern && manager.addJob(patternDetails.getOutputs()[0].getItemStack(), ((EMCCraftingPattern) patternDetails).outputEMC, EqEConfig.emcAssembler.powerPerEMC)) {
        ...
        if (nbttagcompound.hasKey(DISPLAY_TAG)) {
            ...
            for (int i = 0; i < displayStacks.size(); i++) {
                if (invList.hasKey(STACK_TAG + i)) {
                    displayStacks.set(i, ItemStack.loadItemStackFromNBT(invList.getCompoundTag(STACK_TAG + i)));
                } else {
                    displayStacks.set(i, null);
                }
            }
        }
    ...
    public String getPlayerUUID() {
        return transmutationItem == null ? null : UUID.fromString(transmutationItem.getTagCompound().getString("OwnerUUID")).toString();
    }
